...
void Array::insert(Array& sArr, int index)
{
    if (index < 0 || index > size)
        throw ArrayException();
    if(size + sArr.size == capacity)
        increaseCapacity(size + sArr.size);
    for(int j = size - 1; j >= index; j--)
        ptr[j + sArr.size] = ptr[j];
    memcpy(ptr + index, sArr.ptr, sArr.size * sizeof(int));
    size += sArr.size;
}
